...
void EmitQuard1(){
    ...
    vec3 b = vec3(P3) - vec3(P2);
    vec3 normal ;
    if(gs_in[0].Dimension.y < 0 ){
       normal= normalize(cross(a,b));////使用的是相机坐标系 
    }
    else{
...
void EmitQuard2(){
     ...
     //X->Z,1->2
     vec3 a = vec3(P3) - vec3(P1);//X
     vec3 b = vec3(P3) - vec3(P2);//Z
     vec3 normal;
     if(gs_in[0].Dimension.x < 0){
        normal = normalize(cross(b,a));////使用的是相机坐标系 
     }
     else{
