...
__device__ bool IsInOfCudaNode(float3& OctreeVer, Cutter& Device_Cutter) {
	...
	bool IntheCircle = ((X * X + Y * Y) <= Device_Cutter.CutterSize.y * Device_Cutter.CutterSize.y);//在球头铣刀竖直方向内

	if (IntheCircle && (OctreeVer.z > Device_Cutter.CutterPos.z)) return true;

	bool InTheSphere = (X * X + Y * Y + Z * Z) <= (Device_Cutter.CutterSize.y * Device_Cutter.CutterSize.y);//在球头铣刀半球内
