...
int main()
{
	//Octree Head(Point3f(70, 50, 20), Point3f(70, 50, 20));//设置工件的位置与尺寸
	...
	OctreeCut_Iteration2 Octreecut(Head, Point3f(30, 30, 50), Point3f(50,2, 2));
	Octreecut.ReadPathFile("./res/temp.path");
...
// ---------------------------------------
	...
	Shader ourShader("./res/Octree_DrawPoints_Itera2.vs", "./res/Octree_DrawPoints.fs", "./res/Octree_DrawPoints.gs");//编译着色器Octree_DrawPoints_Itera2
	...
	while (!glfwWindowShouldClose(window))
	{
	   ...
	   */
	   ...
	   //顶点绘制
		...
		glBindBuffer(GL_ARRAY_BUFFER, VBO[2]);//绑定缓冲对象//设置缓冲区需要存储的顶点集，用于后续传送给GPU
		...
		glEnableVertexAttribArray(0);
		glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(RenderData), (void*)(sizeof(Point3f)));//体素的长宽高
		...
		glBufferData(GL_ARRAY_BUFFER, Octreecut.RenderItera.size() * sizeof(int), &Octreecut.RenderItera[0], GL_STATIC_DRAW); //设置缓冲区需要存储的顶点集，用于后续传送给GPU,上传的是每个体素的精细度，用来计算体素的长宽高
		
		glVertexAttribPointer(1, 1, GL_INT, GL_FALSE, sizeof(int), (void*)0);//体素的长宽高
		glEnableVertexAttribArray(1);
		glBindBuffer(GL_ARRAY_BUFFER, 0);
