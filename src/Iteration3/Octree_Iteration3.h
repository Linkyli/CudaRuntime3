...
#include<../src/Cuda/CudaOctreeIteration1.cuh>
...
public:
	...
	CudaBitBricks* HostInitBrick;//用于初始化device端的bricks结构
	CudaBitBricks* DeviceTempBrick;

	vector<CudaBitBricks*> BricksToBeProcess_Host;//存储要放到GPU处理的bricks结构
	
	OctreeToBrick* FirstLookUpTable;//1级映射表
	vector<RenderData> testVertex;//存储bricks中用于渲染的体素
	vector<RenderData> OctreeRenderList;//存储八叉树用于渲染的体素

	byte ChildPosCode[8];
	float Precision[14];

	OctreeCut_Iteration3(Octree& copy, Point3f Center, Point3f CS) :C(Center), CutterSize(CS) {
		Head = new Octree(copy);
		Head->index = 0;
		byte ChildCode;
		byte Mask;

		FirstLookUpTable = (OctreeToBrick*)malloc(8194 * sizeof(OctreeToBrick));//初始化1级查找表
		HostInitBrick = (CudaBitBricks*)malloc(sizeof(CudaBitBricks));//用于初始化新生的GPU的brick；
		for (int i = 0; i < 8; i++) {
			//初始化
			ChildCode = 1;
			Mask = 1;
	...
	*/
	...
	void Loop(Point3f& CutterPos, int& Itera, BoundBox CutterBox) {
		...
		//AddSurfacePointsForBrick512_8_RenderList(Head, Itera,CutterBox);
			...
			if (curr->accuracy > Itera && curr->bricks)
			{
				...
				for (int i = 0; i < BrickLength; ++i) {
					...
					if (!Temp->brick[i]) {
						...
						OctreeVertex = GetOctreeVertexForBrick(0, SubTemp, halfDimension);
						RenderData RD = RenderData(OctreeVertex, pow(2.0, curr->accuracy + 3));
						//RD.Itera.x = pow(2.0, curr->accuracy + 3);
						...
						RD = RenderData(OctreeVertex, pow(2.0, curr->accuracy + 3) * (-1));
						//RD.Itera.x = pow(2.0, curr->accuracy + 3) * (-1);
						testVertex.push_back(RD);
				...
				RenderData RD = RenderData(temp, pow(2.0, curr->accuracy));
				testVertex.push_back(RD);
				temp = GetOctreeVertex(7, curr);
				RD = RenderData(temp, pow(2.0, curr->accuracy) * (-1));
				testVertex.push_back(RD);
				//RenderData RD = RenderData(temp, curr->halfDimension);
                //RD.Itera.x = pow(2.0, curr->accuracy);
                //testVertex.push_back(temp); testVertex.push_back(curr->halfDimension);
                //RD.Itera.x = pow(2.0, curr->accuracy) * (-1);
			}
		}
		else
		{
	...
	void Cutter_Dynamic_Surface_ForCuda(Point3f& CutterPos, int& Itera, BoundBox& CutterBox, byte CurrCode, bool IsSurface,int CurrOctreeNum) {
		if (OctreeLine.empty()) return;
		...
		if (curr->accuracy > Itera && is_intersect)//&& is_intersect
		{
			//如果当前八叉树叶子节点还没有被分配bricks结构，那么在GPU中为它申请一个
			if (curr->bricks == nullptr) {
				//curr->bricks = new Bricks(curr); (CudaBitBricks*)malloc(sizeof(CudaBitBricks));
				cudaMalloc((void**)&(curr->bricks),sizeof(CudaBitBricks));
				HostInitBrick->IsSuface = IsSurface;
				HostInitBrick->ParentNumber = CurrOctreeNum;//注意root节点的初始化
                //当前brick体素含量为4096；16 * 16 * 16
				HostInitBrick->InitPos.x = curr->origin.x - curr->halfDimension.x * 15 / 16;
				HostInitBrick->InitPos.y = curr->origin.y - curr->halfDimension.y * 15 / 16;
				HostInitBrick->InitPos.z = curr->origin.z - curr->halfDimension.z * 15 / 16;
				//初始化
		        cudaMemcpy((CudaBitBricks*)(curr->bricks), HostInitBrick, sizeof(CudaBitBricks), cudaMemcpyHostToDevice);
				//添加二级映射表
				FirstLookUpTable[CurrOctreeNum].BricksPointer = (BricksToRenderIndex*)malloc(BrickLength * sizeof(BricksToRenderIndex));

			}
			//将需要处理的bricks加入待处理数组
			BricksToBeProcess_Host.push_back((CudaBitBricks*)(curr->bricks));
			 //迭代次数达到最大，不可再分割，将要被深入切割的八叉树节点的bricks压入vector；
			return;
		}
		//else Itera += 1;//当前迭代次数加1
			...
			for (int i = 0; i <= 7; ++i) {
				...
				else //只要有一个点在刀具外（无论是刀具全在体素中还是刀具与体素部分相交）就说明该体素节点需要被分割
				{
					...
					for (auto child : curr->children) {
						...
						//CurrOctreeNum * 8 + child->index 为八叉树子节点的编号;CurrOctreeNum为当前节点的八叉树编号，用于索引一级映射表中的渲染下标
						Cutter_Dynamic_Surface_ForCuda(CutterPos, Itera, CutterBox, ChildCode, child->Rendered, CurrOctreeNum * 8 + child->index);
					}
					//Cutter_Dynamic(CutterPos, Itera, CutterBox);
					break;
				}
