...
#include"Octree/OpenGL_Render_Octree.h";
...
int main()
{
	Octree Head(Point3f(70, 46.5, 20), Point3f(70,46.5, 20));//设置工件的位置与尺寸
	camera.Right += 100;//调整相机视角
	OctreeCut_2 Octreecut(Head, Point3f(30, 30, 50), Point3f(500, 3, 3));//设置刀具的位置与尺寸；
...
// ---------------------------------------
	...
	Shader ourShader("./res/Octree_DrawPoints.vs", "./res/Octree_DrawPoints.fs", "./res/Octree_DrawPoints.gs");//编译着色器
	...
	BoundBox TestCutterBox = Octreecut.GetCutterBoundBox(CutterPos);
	//PrepareCut(cudaoctree, CutterPos, Octreecut.CutterSize, TestCutterBox);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		TestCutterBox = Octreecut.GetCutterBoundBox(CutterPos);
		...
		if (((int)t % 10) == 0) {
			...
			Octreecut.CudaAddPoints2(cudaoctree);
		}
		PrepareCut(cudaoctree, CutterPos, Octreecut.CutterSize, TestCutterBox);
		TNum++;
		if (TNum == 20) {
	   ...
	   //顶点绘制
		...
		glBufferData(GL_ARRAY_BUFFER, Octreecut.testVertex.size() * sizeof(Point3f), &Octreecut.testVertex[0], GL_STATIC_DRAW);
