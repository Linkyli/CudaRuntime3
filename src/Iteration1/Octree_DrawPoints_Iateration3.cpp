...
int main()
{
	...
	OctreeCut_Iteration1 Octreecut(Head, Point3f(30, 30, 50), Point3f(50, 2, 2));
...
// ---------------------------------------
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < 8; i++) {
			...
			//顶点绘制
			glBindVertexArray(0);
			glBindVertexArray(VAO[i]);//绑定第一个顶点对象Id到顶点对象，顶点对象包含了顶点属性设置，缓冲对象设置等一系列属性
			glBindBuffer(GL_ARRAY_BUFFER, VBO[i]);//绑定缓冲对象
			//设置缓冲区需要存储的顶点集，用于后续传送给GPU
			glBufferData(GL_ARRAY_BUFFER, Octreecut.RenderList[i].size() * sizeof(RenderData), &Octreecut.RenderList[i][0], GL_STATIC_DRAW);
			//设置顶点位置属性，指定location为0
			glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(RenderData), (void*)0);//体素的一个顶点
			glEnableVertexAttribArray(0);
			glVertexAttribPointer(1, 1, GL_FLOAT, GL_FALSE, sizeof(RenderData), (void*)(sizeof(Point3f)));//体素的长宽高
			...
			glDrawArrays(GL_POINTS, 0, Octreecut.RenderList[i].size());
