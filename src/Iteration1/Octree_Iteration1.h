...
public:
	...
	vector<RenderData> RenderList[8];
	...
	OctreeCut_Iteration1(Octree& copy, Point3f Center, Point3f CS) :C(Center), CutterSize(CS) {
	...
	*/
	...
	void AddSurfacePointsForBrick512_8_RenderList(Octree* curr, int& Itera, BoundBox& CutterBox) {
		for (int i = 0; i < 8;i++) {

			//if (IsIntersect(curr->children[i], CutterBox)) {
				RenderList[i].clear();
				IteraAddSurfacePointsForBrick512_And_8_RenderList(curr, Itera,RenderList[i]);
			//}
		}
	}

	void IteraAddSurfacePointsForBrick512_And_8_RenderList(Octree* curr, int& Itera, vector<RenderData>& SubRenderList) {
		if (!curr->sub) {
		...
		else
		{
			...
			for (auto child : curr->children) {
				//if (child->exist) IteraAddPointsForBrick512(child, Itera);
				if (child && (child->exist))  IteraAddSurfacePointsForBrick512_And_8_RenderList(child, Itera, SubRenderList);
			}
		}
	}
